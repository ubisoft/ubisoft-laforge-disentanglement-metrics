...
'''
    First Experiment of section 5.4: "Modular but not Compact"
'''
...
NB_EXAMPLES = 20000
NB_FACTORS = 4
DISTRIBUTION = [np.random.uniform, {'low': 0., 'high': 2 * np.pi}]
...
def get_factors_codes_dataset():
    ...
    cos = np.cos(factors)
    sin = np.sin(factors)
    codes = np.concatenate((cos, sin), axis=1)
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(PROJECT_ROOT, 'results', 'section5.4_compactness1')
