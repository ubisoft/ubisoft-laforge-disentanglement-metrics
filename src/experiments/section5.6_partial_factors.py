...
'''
    Experiment of section 5.6: "When Factors Partially Describe Data"
'''
...
NB_SEEN_FACTORS = [i for i in range(2, NB_FACTORS + 1)]
...
def run_partial_factors_experiment(sub_parser_args):
    ''' Run partial factors experiment using several metrics and save score results
    ...
    for metric in metrics:
        ...
        # initialize arrays
        seen_factors_array = np.asarray(NB_SEEN_FACTORS)
        scores_array = np.zeros((len(seen_factors_array), NB_RANDOM_REPRESENTATIONS, NB_RUNS)).squeeze()
        ...
        # run metric for each nb seen factors
        for line_idx, nb_seen_factors in enumerate(NB_SEEN_FACTORS):
            # get scores using multi-processing
            factors_codes_dataset = partial(get_factors_codes_dataset, nb_seen_factors=nb_seen_factors)
            ...
            # fill arrays for current nb seen factors
            ...
            estimate_required_time(nb_items_in_list=len(seeds) * len(NB_SEEN_FACTORS),
        ...
        if output_dir is not None:
            ...
            for key in metric_scores:
                with open(os.path.join(output_dir, key), 'wb') as output:
                    pickle.dump({f'{key}': (seen_factors_array, metric_scores[key])}, output)
...
def plot_scores(sub_parser_args):
    ...
    for metric, values in scores.items():
        seen_factors_array, scores_array = values[0], values[1]
        scores[metric] = (seen_factors_array,
    ...
    for format in ['png', 'eps']:
        plot_curves(plots, output_file, format=format,
                    x_label='measured factors', y_label='mean score',
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(PROJECT_ROOT, 'results', 'section5.6_partial_factors')
    
    # create parser
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers()
    
    # parser for the "run" command -- run partial factors experiment
    parser_run = subparsers.add_parser('run', help='compute scores')
    parser_run.set_defaults(func=run_partial_factors_experiment)
